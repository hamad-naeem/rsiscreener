...
RSI Screener is a production SaaS that computes RSI for 300+ Binance spot pairs across 15 timeframes, about 4,500 live series, and serves them as one grid that updates within seconds of each candle close. The source is private. This page documents the product and its design; every capture below is the logged-in product.
...
The chart is a canvas renderer written for this product: RSI series, moving average, threshold bands, zoom and pan, and a drawing layer with trendlines, undo/redo, and PNG export. Existing charting libraries didn't support live RSI overlays combined with a drawing state model, so none is used.
